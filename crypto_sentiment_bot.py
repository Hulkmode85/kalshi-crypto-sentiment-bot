...
try:
    if _brier_scorer_available:
        _brier_scorer.record(predicted_prob=locals().get("predicted_prob", locals().get("entry_price", 50)) / 100.0 if locals().get("predicted_prob", locals().get("entry_price", 50)) > 1 else locals().get("predicted_prob", 0.5), actual_outcome=1.0 if locals().get("won", locals().get("pnl", 0) > 0) else 0.0, asset=locals().get("asset", "crypto-sentiment"))
except Exception:
    pass
try:
    if _data_pipeline_available:
        _data_pipeline.record_snapshot({"bot": "crypto-sentiment", "event": "resolution", "pnl": locals().get("pnl", 0), "ts": time.time()})
except Exception:
    pass
...
class KalshiClient:
    ...
    try:
        if _pre_trade_validator_available:
            _ptv = validate_pre_trade({"ticker": locals().get("ticker", ""), "side": locals().get("side", ""), "bot": "crypto-sentiment"})
